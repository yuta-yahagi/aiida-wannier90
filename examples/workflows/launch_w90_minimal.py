...
################################################################################
"""Example script to launch a MinimalW90WorkChain for GaAs."""
import os
...
from aiida.orm import Dict, Group, Str
...
def get_static_inputs():
    ...
    kpoints_scf_mesh = 4
    kpoints_scf.set_kpoints_mesh([kpoints_scf_mesh, kpoints_scf_mesh, kpoints_scf_mesh])
...
def get_or_create_pseudo_family():
    ...
    pseudos_filenames = ["As.pbe-n-rrkjus_psl.0.2.UPF", "Ga.pbe-dn-kjpaw_psl.1.0.0.UPF"]
    ...
    while not group_created:
        ...
        family_name = f"{family_name_prefix}-{safe_counter}"
        group, group_created = Group.objects.get_or_create(
            label=family_name,
            type_string=upf.UPFGROUP_TYPE,  # pylint: disable=no-member # TODO: fix
        )
    ...
    group.description = (
        "Automatic group created by the aiida-Wannier90 minimal workflow example"
    )
    # Put the pseudos in the group
    group.add_nodes(pseudo_nodes)

    print(
        f"Created new UPF group '{family_name}' with nodes {', '.join(str(node.pk) for node in pseudo_nodes)}"
    )
    return family_name
...
def run_wf(pwscf_code, pw2wannier90_code, wannier_code):
    ...
    run(
        ...
        pseudo_family=Str(pseudo_family_name),  # Str('SSSP_efficiency_pseudos'),
        wannier_code=wannier_code,  # load_code('wannier90-3-desktop@localhost'),
        pw2wannier90_code=pw2wannier90_code,  # load_code('pw2wannier90-6.4-release@localhost'),
        **static_inputs,
    )
